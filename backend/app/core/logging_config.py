...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        agent_color = (
            self.AGENT_COLORS.get(agent_name, Colors.WHITE)
            if agent_name
            else Colors.WHITE
        )
...
class AgentLogger:
    ...
    def _log(
        self, level: int, message: str, run_id: str | None = None, **kwargs: Any
    ) -> None:
    ...
    def log_input(
        self, run_id: str, label: str, content: str, max_length: int = 200
    ) -> None:
    ...
    def log_output(
        self, run_id: str, label: str, content: str | list | dict, max_length: int = 200
    ) -> None:
