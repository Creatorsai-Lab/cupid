...
class TestStreamingPipeline:
    ...
    async def test_streaming_yields_updates(self):
        ...
        personalization_update = next(
            (u for u in updates if "personalization" in u["agents_completed"]), None
        )
        assert personalization_update is not None
        assert personalization_update["queries_count"] > 0, (
            "Personalization should generate queries"
        )
    ...
    async def test_supervisor_rejection_stops_pipeline(self):
        ...
        assert len(updates) == 1, (
            f"Expected 1 update (supervisor only), got {len(updates)}"
        )
