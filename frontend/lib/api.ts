...
export const profileApi = {
  get: () => request<ProfileData | null>("/api/v1/profile"),
...
export const trendsApi = {
  getNews: (refresh: boolean = false): Promise<TrendsResponse> =>
    requestRaw<TrendsResponse>(`/api/v1/trends/news${refresh ? "?refresh=true" : ""}`),
};
...
export const connectionsApi = {
  ...
  startYouTube: (): Promise<ConnectionStartResponse> =>
    requestRaw<ConnectionStartResponse>("/api/v1/connections/youtube/connect"),
...
export const insightsApi = {
  list: (): Promise<SummaryResponse[]> => requestRaw<SummaryResponse[]>("/api/v1/insights/"),

  summary: (connectionId: string): Promise<SummaryResponse> =>
    requestRaw<SummaryResponse>(`/api/v1/insights/${connectionId}/summary`),

  timeseries: (connectionId: string, rangeDays: number = 30): Promise<TimeSeriesResponse> =>
    requestRaw<TimeSeriesResponse>(
      `/api/v1/insights/${connectionId}/timeseries?range_days=${rangeDays}`,
    ),

  topContent: (connectionId: string, limit: number = 10): Promise<TopContentResponse> =>
    requestRaw<TopContentResponse>(`/api/v1/insights/${connectionId}/top-content?limit=${limit}`),

  heatmap: (connectionId: string): Promise<HeatmapResponse> =>
    requestRaw<HeatmapResponse>(`/api/v1/insights/${connectionId}/heatmap`),
};
...
export const historyApi = {
  list: (limit: number = 20, offset: number = 0): Promise<HistoryListResponse> =>
    requestRaw<HistoryListResponse>(`/api/v1/history/?limit=${limit}&offset=${offset}`),
...
export const earnApi = {
  ...
  getProfile: (): Promise<EarnProfile> => requestRaw<EarnProfile>("/api/v1/earn/profile"),
