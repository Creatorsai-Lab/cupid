...
export function PostingHeatmap({ data }: { data: HeatmapResponse }) {
    ...
    return (
        ...
        >
            ...
            <div className="overflow-x-auto">
                <div className="inline-block">
                    ...
                    {days.map((dayLabel, dayIdx) => (
                        <div key={dayLabel} className="flex gap-1 mb-1 items-center">
                            <div
                                className="w-9 text-right pr-2 text-[11px]"
                                style={{ color: "var(--color-muted)", fontFamily: "var(--font-body)" }}
                            >
                                {dayLabel}
                            </div>
            ...
            {/* Legend */}
            <div className="flex items-center gap-2 mt-4 text-[10px]" style={{ color: "var(--color-muted)", fontFamily: "var(--font-body)" }}>
                <span>Less</span>
...
function Cell({
    ...
    return (
        <div
            className="w-9 h-9 rounded-md cursor-default transition-transform hover:scale-110"
    ...
    for (let day = 0; day < 7; day++) {
        ...
        for (let block = 0; block < 8; block++) {
            ...
            const matching = cells.filter(
                (c) =>
                    c.day_of_week === day &&
                    c.hour >= blockStartHour &&
                    c.hour < blockStartHour + 3
            );
            if (matching.length === 0) {
                row.push({ avg_views: 0, post_count: 0 });
            } else {
                const totalCount = matching.reduce((s, c) => s + c.post_count, 0);
                const totalViews = matching.reduce(
                    (s, c) => s + c.avg_views * c.post_count,
                    0
                );
                row.push({
